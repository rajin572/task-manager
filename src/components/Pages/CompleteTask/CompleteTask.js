...
const CompleteTask = () => {
  ...
  useEffect(() => {
    fetch(
      `https://task-manager-server-dusky.vercel.app/myCompleteTask?email=${user?.email}`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleDelete = (id) => {
    ...
    if (proceed) {
      fetch(
        `https://task-manager-server-dusky.vercel.app/myCompleteTask/${id}`,
        {
          method: "DELETE",
        }
      )
      ...
      <div className=" min-h-screen mt-10">
        ...
        <div>
          <div className="myCompleteTaskTable bg-gray-200 mx-auto lg:w-[90%] md:w-[95%] sm:w-full font-bold text-center">
            ...
            <p className="myCompletetaskName border-2  border-black">
              Task Name
            </p>
